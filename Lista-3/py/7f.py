...
if __name__ == "__main__":
    ...
    var = 1
    m = 0
    x = np.random.uniform(low=-1., high=1., size=trials)

    y = x**2
    ty = lambda y: 1/(2*np.sqrt(y))

    epdf, bins_centers = pdf_empirical_evaluation(y, nbins=1000)

    plt.clf()
    plt.plot(bins_centers[:len(epdf)], epdf, label='Empirical', linewidth=5)
    plt.plot(bins_centers[:len(epdf)], ty(bins_centers[:len(epdf)]), 'r', label='Theoretical', linewidth=2)
    plt.ylim([0, 40])
    plt.xlabel('$x$')
    plt.ylabel('$f_Y$')
    plt.minorticks_on()
